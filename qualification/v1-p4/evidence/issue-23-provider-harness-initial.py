...
def run(record, checkpoint, output, timeout):
    ...
    from broodling.containment import confirm_ceased
    ...
    try:
        ...
        if not confirm_ceased(case.path):
            raise RuntimeError("completed runtime has not confirmed physical cessation")
        safe_cleanup = True
        record["finalFiles"] = {
