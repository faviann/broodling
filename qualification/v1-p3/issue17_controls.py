...
async def execute(output):
    ...
    record = {
        ...
        "scope": {
            ...
            "testGraphDeviations": "Two of the fourteen cases submit a modified graph, and each carries its own testOnlyGraphDeviation describing what it changed: hang-initial_review shortens that node's timeoutMs to 250 so the hang terminates inside the campaign, and widened-binding-canary adds a forbidden raw-finding findingContent input and binding to repair, in order to show that the raw-finding isolation assertion would catch such a binding. Every other case carries an explicit null and submits the exact product graph. The product graph itself is never mutated; each deviation is built from a fresh copy. The acceptanceChecks entry graph_deviations_are_declared enforces only that a declaration is present exactly when the submitted graphSha256 differs from the product graph's -- so an undeclared future deviation fails the campaign. The wording of each declaration is descriptive and is not machine-compared against the submitted bytes; the recorded per-case graphSha256 is the authoritative record of what ran.",
            "witnessScope": "Issue #45 reduced this campaign to a minimal discriminating set of real Zeroshot runs: 38 became 14. Structural fail-closed topology -- every executable occurrence caught on its own authored route -- plus repair-input isolation, sticky-obligation ownership and diagnostic non-authority are asserted directly against the authored graph by tests/test_assurance_graph_structure.py, which runs in the default regression lane. Two W3 demonstrations are no longer run here because a stronger real witness exists in the same lane: required-evidence removal is covered by the #18 campaign's missing-initial and missing-renewed against the real deterministic leaf, and resolution-by-omission is the omitted-signal rule witnessed by missing-initial_review. Retained historical records are unchanged; this field describes the current run only.",
        },
        "build": build
