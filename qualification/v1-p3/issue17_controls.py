...
async def execute(output):
    ...
    record = {
        ...
        "scope": {
            ...
            "witnessScope": "Issue #45 reduced this campaign to a minimal discriminating set of real Zeroshot runs: 38 became 8. Fail-closed topology -- every executable occurrence caught on its own authored route -- plus repair-input isolation, sticky-obligation ownership, authority ownership and diagnostic non-authority are asserted directly against the authored graph by tests/test_assurance_graph_structure.py, which runs in the default regression lane. Eight W3 demonstrations are no longer run here. The clean route and the found/adjudicated/repaired/resolved route are no longer run here either: the #18 campaign's valid and repair-renewed take the same two routes through the exact product graph AND runtime -- this campaign substitutes every runtime binding's model and connections -- while additionally driving the real deterministic evidence leaf at both occurrences, so they are the stronger witness of the same execution. Route order, the adjudicator receiving the bytes the review emitted as findingContent, and the repair handoff carrying contract/directive/directiveContent and no raw finding are asserted there. Fault location is likewise not a claim -- the authored graph catches every executable occurrence on its own route -- so process death is now witnessed at implement, the first executable node, and every response defect at initial_review, the first model node declaring both a signal and an output payload; that a control error inside the loop stops the loop rather than buying another round is round_complete's error authored in both the loop's until and post_repair_bound_route, asserted against the graph. Required-evidence removal and resolution-by-omission have stronger or equal witnesses in the same lane: the #18 campaign's missing-initial against the real deterministic leaf, and missing-initial_review, whose response is now valid apart from the omitted signal. Removal at the renewed occurrence is not run anywhere: the authored graph carries the same missing->fail branch at both evidence occurrences, asserted branch for branch, and that fresh evidence really executes after a mutation is witnessed by repair-renewed. The other two were synthetic: the widened-binding canary mutated the graph to create the very binding the authored graph is asserted not to have, and its runtime premise -- that a bound state path is delivered -- is witnessed on the unmodified graph by the #18 campaign's repair-renewed, where the bytes the review emitted as findingContent reach adjudicate_authority (the_reviews_actual_finding_reaches_the_adjudicator); repair-resolve, which used to carry that premise here, is itself no longer run; an authority-claiming review response is refused by the runtime as a contract violation, measured, so a run of it duplicated malformed-response rejection while the guarantee itself is that guards name the node whose signal they read. Refusal and final-assessment failure routing is Broodling-owned structural behaviour under #45, and tests/test_assurance_graph_structure.py asserts both final routes branch for branch: the gap and refused guards, the semantic_gap and authority_gap sinks they reach, and acceptance reachable only as the fall-through once both are taken out. The single runtime premise those two runs added -- that a non-accepted signal label reaches the authored sink rather than falling through -- is witnessed by the retained sticky-exhaust on resolution_authority's open_d1, and at a final assessor itself by the #18 campaign's wrong-population, which signals gap and fails semantic_gap on the exact product graph. The retained hang case records reaching the intentional hang from inside the leaf, so the witness distinguishes a provider terminated inside its hang from one the runtime timed out before the execution got that far. The original canary and authority-claim evidence is retained unchanged in the preserved issue-17-controls.json. This entrypoint no longer makes two further real Zeroshot executions of its own outside the eight cases, as earlier records under admittedProductSubmissions show it doing: admitted product submissions of the clean and repair-resolve routes through the P3 coordinator. Those two routes now belong to the #18 campaign's valid and repair-renewed, which submit through the same coordinator on the exact product graph and runtime, and the restart-and-retry property they additionally carried -- a request replayed unchanged after graph-owned mutation, and a correlated retry that never dispatches a second run -- is asserted without Zeroshot by tests/test_assurance_submission.py in the default regression lane. This record therefore contains exactly the eight cases run, and this field describes the current run only.",
        },
        "build": build
