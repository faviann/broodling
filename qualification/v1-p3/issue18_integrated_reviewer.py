#!/usr/bin/env python3
"""Integrated reviewer controls through the admitted product submission path.
    ...
    return criterion(
        ...
        mechanical_evidence=MechanicalEvidence(
            argv=(
                ...
                "-c",
                (
                    "from pathlib import Path; p=Path('candidate.txt'); "
                    "s=p.read_text(); print('actualCandidate=' + repr(s)); "
                    "print('check=C1_FROM_IMPLEMENT in candidate.txt'); "
                    "print('PASS' if 'C1_FROM_IMPLEMENT' in s else 'FAIL')"
                ),
            ),
            materials=("evidence/raw.txt", "candidate.txt"),
        ),
...
def main(output):
    ...
    record = {
        ...
        "scope": "Integrated reviewer profile controls through actual Contract-declared deterministic evidence leaf and unchanged qualified real model reviewers.",
        ...
        "testOnlySubstitution": "Product launcher executes deterministic evidence occurrences. Profile executable dispatches controlled remaining non-reviewer leaves and forwards real reviewers unchanged to qualified CLI; contaminated control deliberately appends six forbidden context tokens after graph prompt construction. No product graph/runtime/profile settings change.",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "qualification/v1-p3/evidence/issue-18-integrated-reviewer.json",
    )
