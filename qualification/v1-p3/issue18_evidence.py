...
def main(output):
    ...
    record = {
        ...
        "witnessScope": "Issue #45 reduced this campaign to one real run per distinct integration assumption. The five other semantic-mismatch permutations differed only in candidate bytes on one route; their Broodling-owned claim — every mismatch dimension and the frozen population reach the assessor byte for byte through the real deterministic leaf — is asserted without Zeroshot by tests/test_evidence_material_fidelity.py. The sticky scenario was also removed: sticky obligations across rounds and the exhausted bound are graph behaviour witnessed through real Zeroshot by the #17 campaign's sticky-exhaust, and the evidence-specific claim -- that a directive raised from real deterministic evidence is still open and still carrying its payload when the round's resolution authority runs, and is closed by that authority rather than by the fresh clean review beside it -- is asserted inside repair-renewed, which already runs a repair round. The renewed missing-evidence scenario was removed for the same reason: missing-initial already witnesses real missing-material behaviour through the product evidence boundary, repair-renewed already witnesses fresh evidence executing after a mutation against the repaired candidate, and the authored graph carries the same missing->fail branch at both evidence occurrences, asserted branch for branch by tests/test_assurance_graph_structure.py. A second real removal added no dependency behaviour beyond those three premises. This campaign also took over the clean and repaired single-round routes from the #17 controls, which substitute every runtime binding's model: valid and repair-renewed run them on the exact product graph and runtime while driving the real deterministic evidence leaf, and now assert route order, the adjudicator receiving the bytes the review emitted as findingContent, and the repair handoff. Retained historical records are unchanged.",
