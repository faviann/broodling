...
public sealed class GitHubRepositoryError(string message)
...
public sealed class GitHubRepositoryCredentials
{
    public GitHubRepositoryCredentials(string token)
    {
        if (string.IsNullOrWhiteSpace(token) || token.Contains('\0'))
            throw new GitHubRepositoryError("Configured GitHub repository credentials are required.");
        Token = token;
    }
...
/// target branch are selected here.
/// </summary>
public sealed class GitHubRepositorySource(string executable = "gh", string gitExecutable = "git")
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (reference.Host != "github.com")
            throw new GitHubRepositoryError("Repository preparation requires github.com.");
        ...
        GitHubRepositoryCredentials credentials, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (repository.ValueKind != JsonValueKind.Object
                ...
                throw new GitHubRepositoryError("GitHub repository identity does not match the Work Unit.");
            if (!StringValue(repository, "node_id", out var repositoryIdentity)
                ...
                throw new GitHubRepositoryError("GitHub repository identity does not match the Work Unit pin.");
            if (!StringValue(repository, "default_branch", out var branch) || !ValidBranchName(branch))
                throw new GitHubRepositoryError("GitHub repository has no supported default branch.");
            if (!StringValue(repository, "clone_url", out var cloneUrl)
                || !CanonicalCloneUrl(cloneUrl, reference))
                throw new GitHubRepositoryError("GitHub repository has no supported authenticated clone URL.");
    ...
    private async Task<byte[]> RunForgeAsync(IReadOnlyList<string> arguments,
        GitHubRepositoryCredentials credentials, CancellationToken cancellationToken)
    {
        var result = await RunProcessAsync(executable, arguments, null, credentials, allowCredentials: true, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (allowCredentials)
        {
            process.StartInfo.Environment["GIT_CONFIG_COUNT"] = "1";
    ...
    private static string RepositoryRoot(string path)
    {
        if (!Path.IsPathFullyQualified(path))
            throw new GitHubRepositoryError("The configured repository root must be absolute.");
        string resolved;
        try { resolved = PhysicalPaths.Resolve(path); }
        catch (IOException error) { throw new GitHubRepositoryError(error.Message); }
        if (PhysicalPaths.IsWithinTemporaryRoot(resolved) || PhysicalPaths.IsWithinDisposable(resolved))
            throw new GitHubRepositoryError("The configured repository root must be durable and owned by the service.");
        return resolved;
    }
