...
server created, with the stopped-target check supplied by the host, though such a
successor has no dispatch path yet (#210); CLI Attempts keep them in the release
artifact. Remaining #100 intent includes
...
successor is admitted and prepared only under the pause and, for a CLI Attempt,
dispatches only after release (a processing-server successor has no dispatch path
yet, #210). A revision (#124) likewise requires every dispatched Attempt of its Work
