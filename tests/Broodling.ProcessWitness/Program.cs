...
internal sealed class CrashTransport(INativeTransport inner, string mode) : INativeTransport
{
    ...
    public Task<NativeProgress> StatusAsync(NativeLocator locator, string runId, CancellationToken cancellationToken = default) => inner.StatusAsync(locator, runId, cancellationToken);
}
