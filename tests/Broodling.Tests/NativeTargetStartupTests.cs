...
public sealed class NativeTargetStartupTests
{
    ...
    {
        await using var target = await Target.Create(await TargetImage.Direct.Value);
        ...
        var list = await DockerCommand("exec", target.Container, "zeroshot", "list", "--target", "broodling");
    ...
    public async Task ConfiguredLocationsAndNonemptyInitializationAreRefused()
    {
        ...
        var missing = await DockerCommand(["run", "--rm", "--network", "none", await TargetImage.Direct.Value, .. Target.Arguments]);
    ...
    private sealed class Target(string image) : IAsyncDisposable
    {
        ...
        internal async Task<Result> RunWith(string[] options, string[] arguments) =>
            await DockerCommand(["run", "--rm", "--name", Container, "--network", "none", .. Mounts, .. options, image, .. arguments]);
        ...
        internal async Task Start() => RequireSuccess(await DockerCommand(["run", "--detach", "--name", Container, "--network", "none", .. Mounts, image, .. Arguments]));
        internal async Task Stop() => RequireSuccess(await DockerCommand("rm", "--force", Container));
        internal async Task<string> Discover()
        {
            for (var retry = 0; retry < 60; retry++)
            {
                var result = await DockerCommand("exec", Container, "node", "-e", "fetch('http://127.0.0.1:18770/.well-known/zeroshot-native-v2').then(r=>r.text()).then(console.log).catch(()=>process.exit(1))");
                if (result.Code == 0) return result.Output;
                await Task.Delay(100);
            }
            throw new InvalidOperationException("Target never served: " + (await DockerCommand("logs", Container)).Error);
        }
        ...
        {
            await DockerCommand("rm", "--force", Container);
            RequireSuccess(await DockerCommand("volume", "rm", id + "-state", id + "-home"));
        }
    }
}
