...
| HTTP send gates, intent before discovery, exact acknowledgement, retained conflict, concurrent/late replies and killed HTTP callers | `HttpDispatchTests`, `DispatchProcessTests`: loopback stock-target stand-in, [HTTP dispatch](../docs/implementation/zeroshot-native-integration.md#http-dispatch-and-acknowledgement) |
