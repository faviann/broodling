...
| Exact-submission preparation over real capture, Git and SQLite with controlled GitHub and gateway peers: one owner per submission with independent submissions, takeover after the first caller cancels, a proposal interrupted after capture repeated against the frozen bundle, a Contract committed before its decision decided without proposing again, and retained findings, cancellation or a retryable failure as results | `SubmissionPreparationTests`: [submission preparation](../docs/implementation/dotnet-contract-admission.md#submission-preparation) |
