#!/usr/bin/env python3
"""Prepare v5 R01 inputs without admission, Attempt provisioning or execution.

Default operation is offline. --github-issue separately reads an existing private
disposable fixture and records unadmitted product inputs. It preserves v3's
disposable-repository description marker, not its execution profile. Neither
...
def slots() -> dict:
    return {
        ...
        "slots": [{
            ...
            "reason": "Awaiting v5 prerequisites and separately authorized first admission",
...
def github_inputs(repository: str, issue_number: int, source: Path,
    ...
    if (not remote["private"] or remote["description"] != setup.DESCRIPTION
...
def prepare(*, state_dir: Path, evidence: Path, repository: str,
    ...
    for directory, marker in ((state, "setup-v5.json"), (evidence, "slots.json")):
        if directory.exists() and any(directory.iterdir()) and not (directory / marker).is_file():
            raise RuntimeError("existing directory has no v5 setup identity; choose a fresh directory")
    ...
    with (state / ".setup.lock").open("a") as lock:
        ...
        setup.record(state / "setup-v5.json", binding)
        ...
        facts.update(
            ...
            protocol_sha256=hashlib.sha256(
                setup.frozen("evaluation/p5/v5/protocol.md", FREEZE)
            ).hexdigest(),
        )
