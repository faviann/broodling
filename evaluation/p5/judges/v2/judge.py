"""p5-judge-v2 evaluation-only checks; separate from p5-native-pr-v2.

Never import this into Broodling or give it to a worker.
...
def cases(task):
    """V1 checks plus the demonstrated T1 falsey-string regression."""
    if task == "T1":
        return [
            ("minimum", ("1",), 1, None),
            ...
            *[(f"type-{i}", (value,), None, "TypeError") for i, value in enumerate((80, True, None))],
            ("valid-falsey-str-80", (FalseyString("80"),), 80, None),
        ]
    if task == "T2":
        return [
            ("empty", ([],), [], None),
...
# Exact frozen v1 reference, retained as a targeted negative calibration control.
OLD_T1_REFERENCE = '''def parse_port(text):
    if not isinstance(text, str):
        raise TypeError("string required")
    if not text or any(c not in "0123456789" for c in text):
        raise ValueError("ASCII digits required")
...
def self_test():
    base = Path(__file__).resolve().parents[2].joinpath("v1/fixture/tiny.py").read_bytes()
    argument = cases("T1")[-1][1][0]
    for value in (argument, copy.deepcopy(argument)):
        assert type(value) is FalseyString and not bool(value)
        assert str.__str__(value) == "80"
    observations = []
    ...
    old_reference = check_source("T1", OLD_T1_REFERENCE)
    failures = [row["id"] for row in old_reference if not row["pass"]]
    observations.append({"task": "T1", "control": "v1-reference",
                         "expected_pass": False, "observed_pass": not failures,
                         "expected_failures": ["valid-falsey-str-80"],
                         "observed_failures": failures})
    ok = all(row["expected_pass"] == row["observed_pass"] for row in observations)
    ok = ok and failures == ["valid-falsey-str-80"]
    print(json.dumps({"evaluator": VERSION, "calibration_pass": ok,
                      "controls": observations}, indent=2))
...
def main():
    ...
    try:
        ...
        print(json.dumps({"evaluator": VERSION, "judgment": "INDETERMINATE", "error": str(error)}))
        return 2
    ...
    print(json.dumps({"evaluator": VERSION, "task": args.task,
                      "baseRevision": B1, "headRevision": head,
